    ...
    */
    ...
    public static String getPath(final Uri uri) {
        try {
            ...
            if (isKitKat && DocumentsContract.isDocumentUri(ContextHolder.getContext(), uri)) {
                ...
                if (isMediaDocument(uri)) {
                    ...
                    return getDataColumn(ContextHolder.getContext(), contentUri, selection, selectionArgs);
                }
            } else if ("content".equalsIgnoreCase(uri.getScheme())) {
                return getDataColumn(ContextHolder.getContext(), uri, null, null);
    ...
    public static void rateInCafeBazaar(Activity activity) {
        Intent intent = new Intent(Intent.ACTION_EDIT);
        intent.setData(Uri.parse(SystemPreferences.MarketPackages.Bazaar.pageAddressPrefix + BuildConfig.APPLICATION_ID));
        intent.setPackage(SystemPreferences.MarketPackages.Bazaar.packageName);
        activity.startActivity(intent);
    }

    private static void rateInMyket(Activity activity) {
        String url= SystemPreferences.MarketPackages.Myket.pageAddressPrefix + BuildConfig.APPLICATION_ID + ";refId=app;end";
    ...
    /**
     * <b>Not working to debug mode. see code comment</b>
     * @param activity
     */
    // in debug mode there s postfix for application_id which makes finding app in google play impossible
    public static void rateInGooglePlay(Activity activity) {
        Intent rateIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse(SystemPreferences.MarketPackages.MARKET_ADDRESS_PREFIX + BuildConfig.APPLICATION_ID));
        ...
        for (ResolveInfo otherApp: otherApps) {
            ...
            if (otherApp.activityInfo.applicationInfo.packageName.equals(SystemPreferences.MarketPackages.GooglePlay.packageName)) {
        ...
        if (!marketFound) {
            Intent webIntent = new Intent(Intent.ACTION_VIEW,
                    Uri.parse(SystemPreferences.MarketPackages.GooglePlay.pageAddressPrefix + BuildConfig.APPLICATION_ID));
            activity.startActivity(webIntent);
        }
    }

    // TODO: horrible code! Refactor it for God's sake.
    public static void rateApplication(Activity activity, SystemPreferences.MarketPackages market) {
        // if the suggested market is not installed, rate it in order of
        // bazaar, myket, googlePlay
        if (market != null && isPackageInstalled(market.packageName, activity)) {
            switch (market) {
                case Bazaar:
                    rateInCafeBazaar(activity);
                    break;
                case Myket:
                    rateInMyket(activity);
                    break;
                case GooglePlay:
                    rateInGooglePlay(activity);
                    break;
            }
        }else if (isPackageInstalled(SystemPreferences.MarketPackages.Myket.packageName, activity)){
            rateInMyket(activity);
        }else if (isPackageInstalled(SystemPreferences.MarketPackages.GooglePlay.packageName, activity)){
            rateInGooglePlay(activity);
        }else if (isPackageInstalled(SystemPreferences.MarketPackages.Bazaar.packageName, activity)){
            rateInCafeBazaar(activity);
        }else{
            // must not happen
            Logger.toastError(R.string.error_no_market_found);
        }
        /*for (String marketPackage : new String[]{})*/
    }
    ...
    /** ask user to grant the needed permission
     *
     * @return true if permission already assigned otherwise false.
     */
    /*public static boolean getPermissionFromUserResult(BaseActivity activity, String permission, int requestCode){
